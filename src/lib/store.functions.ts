...
export const loadAppData = createServerFn({ method: "GET" }).handler(async () => {
  const [settings, sheetSync, productsRes, ordersRes, usersRes, channelsRes] = await Promise.all([
    ...
    db.from("orders").select("*").is("deleted_at", null).order("date", { ascending: false }).limit(5000),
