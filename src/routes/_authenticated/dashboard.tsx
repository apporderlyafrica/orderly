...
function Dashboard() {
  ...
  return (
    <div className="px-4 sm:px-8 py-6 sm:py-10 max-w-7xl">
      ...
      {/* Bande KPI (une ligne, scroll horizontal) */}
      <section className="flex gap-3 overflow-x-auto pb-2 -mx-4 px-4 sm:mx-0 sm:px-0 mb-5 scroll-smooth [scrollbar-width:thin] [scrollbar-color:var(--border)_transparent]">
...
function KpiTile({ icon: Icon, label, value, hint, tone, highlight = false }: { icon: any; label: string; value: number | string; hint: string; tone?: string; highlight?: boolean }) {
  ...
  return (
    <div className={`flex flex-col justify-between min-w-[150px] w-[150px] shrink-0 rounded-xl border px-3.5 py-3 ${highlight ? "bg-primary text-primary-foreground border-primary shadow-[0_8px_24px_-10px_rgba(117,251,144,0.7)]" : "bg-surface border-border"}`}>
      <div className="flex items-center justify-between gap-2">
        <span className={`text-[10px] uppercase tracking-wide font-medium truncate ${highlight ? "opacity-80" : "text-muted-foreground"}`}>{label}</span>
        <Icon className={`h-4 w-4 shrink-0 ${highlight ? "opacity-80" : "text-muted-foreground"}`} />
      </div>
      <div className={`mt-1.5 text-lg font-bold tabular-nums tracking-tight truncate ${highlight ? "" : toneCls}`}>{value}</div>
      <div className={`text-[10px] mt-0.5 truncate ${highlight ? "opacity-80" : "text-muted-foreground"}`}>{hint}</div>
    </div>
  );
}
